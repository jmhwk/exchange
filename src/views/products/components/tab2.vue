	...
	<div>
      ...
      <div v-if="displaynone !=2">
        ...
        <el-row :gutter="30">
          ...
          <el-col :span="12">
            <ul class="lever">
              <li v-for="(i,n) in genrenum" :key="n"  @click="toggle2(n,i.label,numFilter(opneobj2.holdAmount))" :class="{active:active2==n}" >{{i.name}}</li>
            </ul>
          </el-col>
        </el-row> 
      </div>  
...
<!--        <el-row :gutter="30">
		...
		<el-row class="deal" :gutter="30"  v-if="displaynone ==0">
		  <el-col :span="12">
		    ...
		    <div class="rate flexcenter">
		      <a>可平（{{opneobj1.coinName}}）<label>{{numFilter(opneobj1.amount || 0)}}</label></a>
		      <a>持仓：<label>{{numFilter(opneobj1.holdAmount|| 0)}}</label></a>
		    </div>
		    <input type="button" value="卖出平多" class="lbtns lbtns1" @click="add(4,1,form)">
		  </el-col>
		  <el-col :span="12">
		    <div class="ints">
		      <h5>价格</h5>
		    ...
		    <div class="rate flexcenter">
		      <a>可平（{{opneobj1.coinName}}）<label>{{numFilter(opneobj2.amount || 0)}}</label></a>
		      <a>持仓：<label>{{numFilter(opneobj2.holdAmount || 0)}}</label></a>
		    </div>
		    <input type="button" value="买入平空" class="lbtns" @click="add(3,1,form1)">
		  </el-col>
		</el-row>
    <el-row class="deal" :gutter="30"  v-if="displaynone ==1">
      <el-col :span="12">
        ...
        <input type="button" value="卖出平多" class="lbtns lbtns1" @click="add(4,2,form1)">
      </el-col>
      <el-col :span="12">
        <div class="ints">
          <h5>数量</h5>
        ...
        <div class="rate flexcenter">
          <a>可平（{{opneobj2.coinName}}）<label>{{numFilter(opneobj2.amount || 0)}}</label></a>
          <a>持仓：<label>{{numFilter(opneobj1.holdAmount || 0)}}</label></a>
        </div>
        <input type="button" value="买入平空" class="lbtns" @click="add(3,2,form)">
      </el-col>
    </el-row>
    <el-row class="deal" :gutter="30"  v-if="displaynone ==2">
      <el-col :span="12">
        ...
        <input type="button" value="卖出平多" class="lbtns lbtns1" @click="add(4,3,form)">
      </el-col>
      <el-col :span="12">
        <div class="ints">
          <h5>触发价格</h5>
        ...
        <input type="button" value="买入平空" class="lbtns" @click="add(3,3,form1)">
      </el-col>
    </el-row>
	</div>
</template>
